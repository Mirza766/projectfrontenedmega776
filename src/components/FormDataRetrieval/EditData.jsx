...
import { ArrowRight, DeleteIcon, UploadCloud } from "lucide-react";
import {
  TextField,
  FormControl,
  InputLabel,
...
const updatingData=(data)=>{
  ...
  return (
      ...
      <div className= "relative w-full max-w-xl bg-white/5 backdrop-blur-xl rounded-2xl p-4 shadow-2xl border border-white/10">
        <form className=" w-full  text-left bg-linear-to-r from-gray-900/10 to-gray-800/10 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-2xl border border-white/10" onSubmit={handleSubmit(updatingData)}>
          <Controller
         name='fullName'
         control={control}
 ...
 <button type='submit' className={` text-white mt-4 mb-2 group flex items-center gap-1 w-full  sm:w-auto justify-center rounded-lg text-sm sm:text-base bg-linear-to-b from-blue-600 to-blue-400 font-semibold px:6
                   sm:px-8 sm:py-4 py-3 hover:scale-102 transition-all duration-300 cursor-pointer {isBooked ? "bg-red-900 text-gray-400" : "bg-slate-800 text-blue-400"}`} onClick={()=>navigate("/")}>          
                    <span> Update</span>
                    <UploadCloud className="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform duration-300"/>
                   </button>
                   <button type='submit' className={` text-white mt-4 mb-2 group flex items-center gap-1 w-full  sm:w-auto justify-center rounded-lg text-sm sm:text-base bg-linear-to-b from-blue-600 to-blue-400 font-semibold px:6
                   sm:px-8 sm:py-4 py-3 hover:scale-102 transition-all duration-300 cursor-pointer {isBooked ? "bg-red-900 text-gray-400" : "bg-slate-800 text-blue-400"}`}>          
                    <span> Cancel</span>
                    <DeleteIcon className="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform duration-300"/>
                   </button>
